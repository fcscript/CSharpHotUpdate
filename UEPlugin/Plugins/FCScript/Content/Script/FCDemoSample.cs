...
class FCDemoSample
{
	...
	// key : int, value : int
	public static void  TestTMap_int_int()
	{
		...
		TMapIterator<int, int> it = aID.begin();
		while(it.IsValid())
		{
			os.print("[FCTestScript]key={0}, value={1}", it.key, it.value);
			it.ToNext();
		}
		map<int, int> b = aID.ToMap();
		os.print("------aID.ToMap()-----");
		for(iterator it2 = b.begin(); it2; ++it2)
		{
			os.print("[FCTestScript.ToMap]key={0}, value={1}", it2.key, it2.value);
		}
		os.print("b.Length={0}", b.Length);
		b.RemoveAll();
		b[1] = 10;
		b[2] = 20;
		b[3] = 30;
	...
	public static void  TestTMap_int_string()
	{
	...
	public static void TestCppWrap()
	{
		FFCTestInterface ins = FFCTestInterface.Get();
		ins.HP = 1000;
		print("[FCTestScript]ins.HP={0}, GetHP()={1}", ins.HP, ins.GetHP());
	}
	public static void TestTSet_string()
	{
		TSet<StringW> strSet = new TSet<StringW>(); // 等价于TSet<FString>
		FString aa = UEUtil.AToFString("aa");
		os.print("[FCTestScript]aa={0}", UEUtil.FStringToA(aa))
		strSet.Add(L"aa1");
		strSet.Add(L"bb1");
		strSet.Add(L"cc1");
		os.print("[FCTestScript]set length:{0}", strSet.Length);
		List<StringW>  aNames = strSet.ToArray();
		for(int i =0; i<aNames.Length; ++i)
		{
			os.print("[FCTestScript]{0}:{1}", i, aNames[i]);
		}
		strSet.RemoveAt(0);
		int MaxIndex = strSet.GetMaxIndex();
		os.print("[FCTestScript]set MaxIndex:{0}, length:{1}", MaxIndex, strSet.Length);
		for(int i = 0; i<MaxIndex; ++i)
		{
			int ValidIndex = strSet.ToNextValidIndex(i);
			if(strSet.IsValidIndex(i))
			{
				os.print("[FCTestScript]{0}:{1}, ValidIndex={2}", i, strSet.GetAt(i), ValidIndex);
			}
			else
			{
				os.print("[FCTestScript]{0} is invalid, NextValidIndex={1}", i, ValidIndex);
			}
		}
	}
};