...
void FCDynamicOverrideNative(UObject* Context, FFrame& TheStack, RESULT_DECL)
{
	...
	FCDynamicFunction *DynamicFunction = FFCObjectdManager::GetSingleIns()->ToOverrideFunction(Object, Func, FCDynamicOverrideNative, EX_CallFCOverride);
...
void FCDynamicOverrideBeginBeginPlay(UObject* Context, FFrame& TheStack, RESULT_DECL)
{
	...
	if(ScriptContext && BindInfo)
    {
		bool bUnpackParams = false;
		UFunction* Func = FirstNative(Context, TheStack, bUnpackParams);
		FCDynamicFunction *DynamicFunction = FFCObjectdManager::GetSingleIns()->ToOverrideFunction(Object, Func, FCDynamicOverrideBeginBeginPlay, EX_CallFCBeginPlay);
		if(DynamicFunction)
        {
