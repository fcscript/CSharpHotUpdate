...
public:
	FCDynamicFunction  *RegisterReceiveBeginPlayFunction(UObject *InObject, UClass* Class);

	// 功能：覆盖函数
	FCDynamicFunction  *RegisterOverrideFunc(UObject *InObject, fc_intptr InScriptPtr, const char *InFuncName);
	FCDynamicFunction  *ToOverrideFunction(UObject *InObject, UFunction *InFunction, FNativeFuncPtr InFuncPtr, int InNativeBytecodeIndex);
    FCDynamicFunction  *FindOverrideFunction(UObject *InObject, UFunction *InFunction);
...
protected:
	...
	// ------------------------------	
	typedef stdext::hash_map<UFunction*, FCDynamicFunction*> COverrideFunctionMap;	
